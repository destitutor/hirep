...
public class HangulUtils {
    ...
    private static final int CHOSUNG_START = 0x3131;
    private static final int CHOSUNG_END = 0x314E;
    private static final int JUNGSUNG_START = 0x314F;
    private static final int JUNGSUNG_END = 0x3163;
    private static final int JONGSUNG_START = 0x3164;
    private static final int JONGSUNG_END = 0x3182;
    ...
    private static final char[] JONGSUNG = {
            Character.MIN_VALUE, 'ㄱ', 'ㄲ', 'ㄳ', 'ㄴ', 'ㄵ', 'ㄶ', 'ㄷ', 'ㄹ', 'ㄺ', 'ㄻ', 'ㄼ', 'ㄽ', 'ㄾ', 'ㄿ', 'ㅀ', 'ㅁ', 'ㅂ', 'ㅄ', 'ㅅ', 'ㅆ', 'ㅇ', 'ㅈ', 'ㅊ', 'ㅋ', 'ㅌ', 'ㅍ', 'ㅎ'
    };
    ...
    public static boolean isHangul(char c) {
        return isChosung(c) || isJungsung(c) || isJongsung(c) || isCompleteType(c);
    }
    ...
    public static char getJungsung(char c) {
        if (!isJungsung(c) && !isCompleteType(c)) {
            throw new IllegalArgumentException("입력된 문자에 중성이 없습니다.");
        }
    ...
    public static char getJongsung(char c) {
        if (!isJongsung(c) && !isCompleteType(c)) {
            throw new IllegalArgumentException("입력된 문자에 종성이 없습니다.");
        }
